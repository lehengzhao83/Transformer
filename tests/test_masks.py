...
def test_pad_mask_blocks_pad_tokens() -> None:
    pad_idx = 1
    q = torch.tensor([[2, 5, 1, 1]], dtype=torch.long)
    k = torch.tensor([[7, 1, 9]], dtype=torch.long)
    m = make_pad_mask(q, k, pad_idx=pad_idx)
...
def test_decoder_self_mask_is_pad_and_causal() -> None:
    pad_idx = 1
    tgt = torch.tensor([[2, 4, 5, 1, 1]], dtype=torch.long)
    m = make_decoder_self_mask(tgt, pad_idx=pad_idx)
...
def test_cross_mask_blocks_src_pad() -> None:
    pad_idx = 1
    tgt = torch.tensor([[2, 4, 5]], dtype=torch.long)
    src = torch.tensor([[2, 7, 1, 1]], dtype=torch.long)
    m = make_cross_mask(tgt, src, pad_idx=pad_idx)
